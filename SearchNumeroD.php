...
$error = null; $results = [];
if (!isset($error) && $numeroD === '') { $error = 'Debe indicar NumeroD.'; }

if (!$error) {
        try {
                $pdo = db();
                // Fetch SOLV rows for NumeroD and augment with SAFACT info
                $stmt = $pdo->prepare('SELECT CodClie, FechaE, Status, NumeroD, Solved, CarnetNum2, hasta FROM SOLV WHERE NumeroD = ?');
                $stmt->execute([$numeroD]);
                ...
                if ($rows) {
                        ...
                        $error = 'No se encontraron resultados por NumeroD.';
                }
        } catch (Throwable $e) {
                error_log('[SearchNumeroD] ' . $e->getMessage());
                $error = 'Error interno.';
        }
}

render_header('Resultados por NumeroD', 'piscina.jpg');
?>
