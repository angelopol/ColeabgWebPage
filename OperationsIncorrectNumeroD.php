...
require_auth();
render_header('Operaciones con NumeroD incorrecto', 'piscina.jpg');

$pdo = DataConnect();

$st = $pdo->query("SELECT CodClie, FechaE, NumeroD, Solved, CarnetNum2, hasta FROM SOLV WHERE FechaE IS NULL");
...
<div class="container mx-auto min-h-screen py-10">
    <div class="max-w-5xl mx-auto p-6 bg-slate-900/70 rounded-lg border border-slate-700/50">
        <h1 class="text-2xl text-white font-semibold mb-6">Operaciones con NumeroD incorrecto</h1>
        <?php if (!$rows): ?>
