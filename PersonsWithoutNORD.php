...
require_auth();
render_header('Personas sin Nombre o Dirección', 'piscina.jpg');

$pdo = DataConnect();

// Obtener operaciones con Status=1 y sin Name o Direccion
$sql = "SELECT CodClie, FechaE, NumeroD, Solved, CarnetNum2, hasta FROM SOLV WHERE Status = 1 AND (Name IS NULL OR Direccion IS NULL)";
$stmt = $pdo->query($sql);
...
<div class="container mx-auto min-h-screen py-10">
    <div class="max-w-5xl mx-auto p-6 bg-slate-900/70 rounded-lg border border-slate-700/50">
        <h1 class="text-2xl text-white font-semibold mb-6">Personas sin Nombre o Dirección</h1>
        <?php if (!$rows): ?>
